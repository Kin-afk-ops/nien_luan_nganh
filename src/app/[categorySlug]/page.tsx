...
const FillerProductByCategory = () => {
    ...
    useEffect(() => {
        const fetchCategory = async () => {
            const res = await fetch(`http://localhost:8000/api/categories/${categorySlug}`)
    ...
    useEffect(() => {
        const fectchData = () => {
            fetch(`http://localhost:8000/api/categories/products/${category?.id}?page=${page}&limit=${limit}&sort=${sort}`)
            .then(res => res.json())
            ...
            <main>
                <div className="header">
                    ...
                    <div className="select">
                        ...
                        <select name="Lọc sản phẩm" id="sort_product" value={sort} onChange={handleSortChange} className='select_box'>
