...
const AddCategoryComponent = () => {
  ...
  useEffect(() => {
    ...
    if (newCategory?.attributeId && attribute.length > 0) {
      const foundAttr = attribute.find(
        (attr) => attr.attributeId === Number(newCategory.attributeId)
      );
  ...
  const handleChange = (
    ...
    setNewCategory((prev) => ({
      ...prev,
      [name]:
        name === "parentId" || name === "attributeId"
          ? Number(value) || ""
          : value,
    }));
  };
        ...
        {selectedAttribute !== null ? (
          <ul>
            {selectedAttribute.listDataType.map((detail) => (
              <li key={detail.id} className="item">
                {detail.label}
              </li>
            ))}
          </ul>
